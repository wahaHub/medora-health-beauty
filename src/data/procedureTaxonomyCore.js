...
export const normalizeProcedureLabel = (label) =>
  safeDecodeURIComponent(label)
    .toLowerCase()
    .replace(/[®™©]/g, '')
    .replace(/&/g, ' and ')
    .replace(/\//g, ' ')
...
export const getProcedureSlug = (procedureName) =>
  normalizeProcedureLabel(procedureName)
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-+|-+$/g, '');

const getImplementedProcedurePathValue = (procedureName) => safeDecodeURIComponent(procedureName).trim();

export const getImplementedProcedureGuideUrl = (procedureName) =>
  `/procedure/${encodeURIComponent(getImplementedProcedurePathValue(procedureName))}`;
...
export const getSupportedProcedureOptions = () => {
  const seen = new Set();
  return getAllProcedureOptions().filter((procedure) => {
...
export const getProcedureAreaQueryValue = (procedureName) => {
...
export const getProcedureVideoGalleryUrl = (procedureName) => {
  const params = new URLSearchParams({
    procedure: safeDecodeURIComponent(procedureName),
