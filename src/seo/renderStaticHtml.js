...
export function createSeoPayload(route, data) {
  ...
  if (procedure && route === procedure.guideUrl) {
    return {
      ...
      summary: procedure.shortAnswerSummary || procedure.description,
      sections: [
        { title: 'Short Answer', items: [procedure.shortAnswerSummary].filter(Boolean) },
        { title: 'Overview', items: [procedure.overview].filter(Boolean) },
        { title: 'Benefits', items: procedure.benefits },
        { title: 'Candidacy', items: procedure.candidacy },
        { title: 'Who Should Avoid or Delay', items: procedure.whoShouldAvoid },
        { title: 'Techniques and Approach', items: procedure.techniques },
        { title: 'Procedure Planning', items: [procedure.procedureDescription].filter(Boolean) },
        { title: 'Expected Results', items: [procedure.expectedResults].filter(Boolean) },
        { title: 'Recovery Timeline', items: procedure.recoveryTimeline },
        { title: 'Recovery Tips', items: procedure.recoveryTips },
        { title: 'Risks', items: procedure.risks },
        { title: 'Cost Factors', items: procedure.costFactors },
        { title: 'Complementary Procedures', items: procedure.complementaryProcedures },
        {
          title: 'FAQ',
          items: (procedure.faqItems || []).map((item) => `${item.question} ${item.answer}`),
        },
        { title: 'Medical Disclaimer', items: [procedure.medicalDisclaimer].filter(Boolean) },
      ],
      links: [
